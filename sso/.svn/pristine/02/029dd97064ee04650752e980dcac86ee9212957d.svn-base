...
<html lang="en">
	...
	<body>
					...
					<div class="page-content">
						<form action="${base}/privileges/list" method="get">
						<div class="page-header">
							<div class="form-group">
								<label class="col-sm-1 control-label " >权限名称 </label>
								<span class="input-icon">
									<input type="text" placeholder="请输入权限名称" name="name" <#if entity.name ??> value="${entity.name}" </#if> class="nav-search-input" id="nav-search-input" autocomplete="off" />
									<i class="icon-search nav-search-icon"></i>
								</span>
								<@p.privilege menuAlias="privilege_sys"  priAlias="selete">
								<button class="btn btn-sm btn-primary ">提交</button>
								</@p.privilege>
								<button class="btn btn-sm btn-pink" type="reset" value="Reset">重置</button>
								<@p.privilege menuAlias="privilege_sys"  priAlias="add">
								<a href="${base}/privileges/add" class="btn btn-purple">添加权限+</a>
								</@p.privilege>
							</div>
									...
									<div class="col-xs-12">
										...
										<div class="table-responsive">
											<table id="sample-table-2" class="table table-striped table-bordered table-hover">
												...
												<tbody>
												<#list pages.resultList as privileges>
													<tr id="${privileges.id}">
														<td>${privileges.createDate}</td>
														<td>${privileges.name}</td>
														<td>${privileges.alias}</td>
														<td>${privileges.description}</td>
														<td><#if privileges.isEnabled ==1><span class="label label-sm label-success">启用</span><#else><span class="label label-sm label-warning">禁用</span></#if></td>
														<td>
															<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
																<@p.privilege menuAlias="privilege_sys"  priAlias="update">
																<a href="${base}/privileges/edit?id=${privileges.id}" class="btn btn-xs btn-info" role="button">
																<i class="icon-edit bigger-120"></i>
																</a>
																</@p.privilege>
																<@p.privilege menuAlias="privilege_sys"  priAlias="del">
																<a href="#" onclick="del('${privileges.name}','${privileges.id}')" class="btn btn-xs btn-danger" role="button">
		...
		<script type="text/javascript">
			...
			*/
			function loadInfor(nowPage,totalPage,totalRecords,rows){
			   	var url = "${base}/privileges/list.html";
